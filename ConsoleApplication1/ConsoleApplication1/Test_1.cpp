 ...
 * 取消下行注释以使用demo3多边形
 */
 //vector<Point> vertices = { Point(200, 500), Point(200, 1000), Point(900, 600), Point(1600, 1000), Point(1600, 400), Point(1200, 200), Point(700, 200) };
 /**
 * 取消下行注释以使用demo4箭头
 */
...
int main(int argc, char** argv) {
    ...
    glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB); // 显示模式为RGB显示模式，单缓冲模式
    glutInitWindowPosition(50, 100); //窗口位置
    glutInitWindowSize(windowWidth, windowHeight); // 窗口大小
    glutCreateWindow("Polygon Scan Demo"); // 窗口名称
    init();
    glutDisplayFunc(polygonScan); // 扫面多边形
    glutMainLoop(); // 消息循环相应
    return 0;
}